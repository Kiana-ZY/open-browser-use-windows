...
| 可观测性 | B | CLI / runner / MCP 已给浏览器 RPC 自动补 `request_id`，action plan 和 MCP direct tool call 可写带 risk、session、turn、tab id、耗时和错误的 JSONL trace；`doctor --browser all --json` 和 MCP `doctor` 提供 Chrome/Edge 聚合诊断。 | 继续把 extension / native host 内部事件接入同一 request id，并补 SDK 侧 trace helper。 |
